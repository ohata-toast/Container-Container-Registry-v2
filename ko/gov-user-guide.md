...
Docker 명령줄 도구 없이 NHN Cloud Console에서 태그를 생성할 수 있습니다. 아티팩트 목록 보기 화면에서 태그를 추가할 아티팩트를 선택한 뒤, 하단의 상세 정보 보기 화면에서 **태그** 탭을 선택합니다. 그러면 현재 선택한 아티팩트에 지정된 태그 목록을 볼 수 있습니다. 여기서 **태그 생성** 버튼을 클릭하면 **태그 생성** 팝업이 나타나고, 원하는 태그 이름을 입력하여 새로운 태그를 생성할 수 있습니다.
...
### 레지스트리 웹훅 설정
이미지 변경 사항에 대한 알림을 받고 싶다면 NHN Cloud Console에서 웹훅 설정을 등록할 수 있습니다. 웹훅을 설정할 레지스트리를 선택한 후 하단의 상세 정보 보기 화면에서 **웹훅** 탭을 선택합니다. 이 후 **웹훅 생성** 버튼을 클릭하면 생성할 웹훅의 속성을 지정할 수 있는 팝업이 나타납니다. 필요한 속성을 입력하여 **확인** 버튼을 누르면 웹훅이 호출될 준비가 모두 끝납니다. 현재는 HTTP(S) 호출과 Slack 메신저를 이용한 알림 설정을 지원합니다.

> [참고]
>
> 공공 클라우드에서는 웹훅 설정 시 목적지 포트로 80, 88, 443, 8443 만을 지원합니다.
